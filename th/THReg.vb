...
Namespace th
    ...
    Friend NotInheritable Class THReg
        ...
        Public Shared Function GetProductReg(ByRef Optional ID As String = "") As String
            ...
            End If
            Dim length As Integer = (Math.Round(Conversion.Int(CDbl((CDbl(Strings.Len(expression)) / 3)))))
        ...
        Private Shared Function GetRegChar(ByRef position As Integer) As String
        ...
        Private Shared Function RegGetAlpha(ByRef position As Integer) As String
            Dim regChar As String = THReg.GetRegChar(position)
            If (regChar = "null") Then
                Dim num2 As Integer
                Dim num4 As Integer
                Dim str As String = Conversion.Str((position))
                str = Strings.Mid(str, 2, (Strings.Len(str) - 1))
                Dim strArray As String() = New String((Strings.Len(str) + 1)  - 1) {}
                Dim num As Integer = (Strings.Len(str))
                num2 = 1
                Do While (num2 <= num)
                    strArray(num2) = Strings.Mid(str, num2, 1)
                    num2 = ((num2 + 1))
                Loop
                Dim num3 As Integer = (Strings.Len(str))
                num2 = 1
                Do While (num2 <= num3)
                    num4 = (Math.Round(CDbl((num4 + Conversion.Val(strArray(num2))))))
                    num2 = ((num2 + 1))
                Loop
        ...
        Private Shared Function ValRegString(ByRef RString As String, ByRef number As Integer, ByRef Optional reverse As Boolean = False) As String
            Dim num As Integer
            Dim str As String
            Dim num2 As Integer
            Dim str2 As String
            ...
            Dim str3 As String
            If Not reverse Then
                Dim num4 As Integer = (Strings.Len(CStr(RString)))
                num2 = 1
                Do While (num2 <= num4)
                    str = Strings.Mid(RString, num2, 1)
                    If (num3 = 0) Then
                        num3 = (Strings.Len(CStr(RString)))
                    Else
                        num3 = ((num3 - 1))
                    End If
                    If (num3 < num2) Then
                    ...
                    num = (((Strings.Asc(str) + Strings.Asc(str2)) + ((number + number))))
                ...
                Return str3
            End If
            num2 = ((Strings.Len(CStr(RString)) + 1))
            Do While (num2 > 1)
                num2 = ((num2 - 1))
                str = Strings.Mid(RString, num2, 1)
                If (num3 = 0) Then
